...
$(GOLANGCI_LINT): $(LOCALBIN)
	...
	@if [ -f .custom-gcl.yml ]; then \
		_want="$$(cksum .custom-gcl.yml | awk '{print $$1}')"; \
		if [ "$$(cat "$(GOLANGCI_LINT)-$(GOLANGCI_LINT_VERSION).cksum" 2>/dev/null)" != "$$_want" ]; then \
			...
			printf '%s\n' "$$_want" > "$(GOLANGCI_LINT)-$(GOLANGCI_LINT_VERSION).cksum" && \
			touch "$(GOLANGCI_LINT)-$(GOLANGCI_LINT_VERSION)"; \
		fi; \
	fi
